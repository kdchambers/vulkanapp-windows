...
pub fn normalizePoint(absolute_px: f32, max_value: f32) f32 {
    return -1.0 + ((absolute_px / max_value) * 2.0);
}

pub fn normalizeDist(absolute_px: f32, max_value: f32) f32 {
...
pub fn EventBuffer(comptime EventType: type, comptime buffer_size: usize) type {
    return struct {
        buffer: [buffer_size]EventType,
        count: u32,
        ...
        pub fn write(self: *@This(), event: EventType) !void {
            if(self.count >= buffer_size) {
                return error.BufferFull;
            }
            self.buffer[self.count] = event;
            self.count += 1;
        }
